...
export default async function BundlePage({ params }: PageProps) {
  ...
  // Try both permalink formats to handle different Spree configurations
  const category =
    (await getCategory("categories/2ml-samples").catch(() => null)) ??
    (await getCategory("2ml-samples").catch(() => null));

  const result = category
    ? await getCategoryProducts(category.id, { per_page: 100 }).catch(() => null)
    : null;
