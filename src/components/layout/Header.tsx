...
export async function Header({ rootCategories, basePath, locale }: HeaderProps) {
  ...
  return (
    <div className="relative z-50">
      ...
      <div className="[&>header]:sticky [&>header]:top-0 [&>header]:border-b-0">
        <SearchToggle
          basePath={basePath}
          left={
            ...
            <div className="flex flex-col items-center justify-center py-[15px]">
              <Link href={basePath || "/"}>
                <Image
                  ...
                  height={41}
                  priority
                  style={{ height: "41px", width: "auto", objectFit: "contain" }}
                />
              </Link>
            </div>
          }
      ...
      {/* ── NAV LINKS: sticky below logo row, desktop only, single bottom border ── */}
      <nav className="hidden lg:flex justify-center border-b border-gray-200 bg-white sticky top-16 z-40">
